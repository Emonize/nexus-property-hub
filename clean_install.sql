-- 1. CLEANUP EXISTING BROKEN STATE
...
-- 4. RLS AND SECURITY BYPASS FUNCTIONS
CREATE OR REPLACE FUNCTION public.is_space_owner(space_uuid UUID)
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS(SELECT 1 FROM public.spaces WHERE id = space_uuid AND owner_id = auth.uid());
$$;

CREATE OR REPLACE FUNCTION public.is_space_tenant(space_uuid UUID)
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS(SELECT 1 FROM public.leases WHERE space_id = space_uuid AND tenant_id = auth.uid());
$$;

CREATE OR REPLACE FUNCTION public.is_lease_tenant(lease_uuid UUID)
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS(SELECT 1 FROM public.leases WHERE id = lease_uuid AND tenant_id = auth.uid());
$$;
...
CREATE POLICY users_self ON public.users FOR ALL USING (
...
CREATE POLICY spaces_access ON public.spaces FOR ALL USING (
  owner_id = auth.uid() OR public.is_space_tenant(id) OR EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'admin')
);

CREATE POLICY leases_access ON public.leases FOR ALL USING (
  tenant_id = auth.uid() OR public.is_space_owner(space_id) OR EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'admin')
);

CREATE POLICY payments_access ON public.rent_payments FOR ALL USING (
  tenant_id = auth.uid() OR public.is_space_owner( (SELECT space_id FROM public.leases WHERE id = lease_id) ) OR EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'admin')
);

CREATE POLICY tickets_access ON public.maintenance_tickets FOR ALL USING (
  reporter_id = auth.uid() OR public.is_space_owner(space_id) OR EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'admin')
);

CREATE POLICY trust_self ON public.trust_scores FOR SELECT USING (
  user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.leases WHERE tenant_id = user_id AND public.is_space_owner(space_id)) OR EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role = 'admin')
);
...
CREATE TRIGGER audit_spaces AFTER INSERT OR UPDATE OR DELETE ON public.spaces FOR EACH ROW EXECUTE FUNCTION audit_trigger_func();
CREATE TRIGGER audit_leases AFTER INSERT OR UPDATE OR DELETE ON public.leases FOR EACH ROW EXECUTE FUNCTION audit_trigger_func();
CREATE TRIGGER audit_payments AFTER INSERT OR UPDATE OR DELETE ON public.rent_payments FOR EACH ROW EXECUTE FUNCTION audit_trigger_func();
CREATE TRIGGER audit_tickets AFTER INSERT OR UPDATE OR DELETE ON public.maintenance_tickets FOR EACH ROW EXECUTE FUNCTION audit_trigger_func();
CREATE TRIGGER audit_trust AFTER INSERT OR UPDATE OR DELETE ON public.trust_scores FOR EACH ROW EXECUTE FUNCTION audit_trigger_func();
